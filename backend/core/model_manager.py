...
class ModelManager:
    ...
    def __init__(self, config_loader: ConfigLoader):
        ...
        # 安全配置
        self.verify_ssl = config_loader.getboolean(
    ...
    def _init_local_config(self):
        ...
        self.default_max_tokens = self.config_loader.getint(
            "ai_model", "local.max_tokens", 512
        )
        self.default_temperature = 0.3  # 本地模型使用更低温度

        # 获取用户配置的模型名称（优先）
        configured_model = self.config_loader.get(
            "ai_model", "local.model_name", "local"
        )
    ...
    def get_model_limits(self) -> Dict[str, Any]:
        ...
        if self.mode == ModelMode.LOCAL:
            # 获取模型大小分类
            model_category = getattr(self, "model_size_params", {}).get(
                "category", "small"
            )
    ...
    def generate(
        ...
        try:
            ...
            # 调用生成
            for chunk in self._chat_generate(
                redacted_prompt,
