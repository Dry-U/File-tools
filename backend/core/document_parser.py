...
class DocumentParser:
        ...
        try:
            ...
            if textract:
                try:
                    ...
                    self.logger.warning(
                        f"无法使用textract解析PPTX文件 {file_path}: {str(te)}"
                    )
    ...
    def __init__(self, config_loader):
        ...
        # 这些限制用于防止内存溢出
        max_file_size_mb = config_loader.getint(
            "file_scanner", "max_file_size", self.DEFAULT_MAX_FILE_SIZE_MB
        )
    ...
    def _clean_text(self, text):
        ...
        text = re.sub(
            r"[\ue000-\uf8ff\U000f0000-\U000fffff\U00100000-\U0010ffff]", "", text
        )
    ...
    def extract_metadata(self, file_path):
        ...
        try:
            ...
            metadata["创建时间"] = datetime.datetime.fromtimestamp(
                stat_info.st_ctime
            ).strftime("%Y-%m-%d %H:%M:%S")
            metadata["修改时间"] = datetime.datetime.fromtimestamp(
                stat_info.st_mtime
            ).strftime("%Y-%m-%d %H:%M:%S")
            metadata["访问时间"] = datetime.datetime.fromtimestamp(
                stat_info.st_atime
            ).strftime("%Y-%m-%d %H:%M:%S")
    ...
    def _parse_docx(self, file_path):
        ...
        try:
            ...
            if file_size > max_size:
                self.logger.warning(
                    f"Word文档过大，跳过解析 {file_path}: {file_size} bytes"
                )
                ...
                try:
                    ...
                    try:
                        ...
                        if _file_size > _max_size:
                            return (
                                f"错误: Word文档过大 ({_file_size} bytes)，已跳过解析"
                            )
                    except OSError:
                        pass

                    content = textract.process(file_path).decode(
                        "utf-8", errors="ignore"
                    )
                    ...
                    self.logger.warning(
                        f"无法使用textract解析Word文档 {file_path}: {str(te)}"
                    )
    ...
    def _parse_doc_win32(self, file_path):
        ...
        try:
            ...
            self.logger.warning(
                f"Win32直接解析.doc失败 {file_path}: {str(e)}，尝试转换格式..."
            )
    ...
    def _parse_text(self, file_path):
        ...
        try:
            ...
            if file_size > max_size:
                self.logger.warning(
                    f"文本文件过大，跳过解析 {file_path}: {file_size} bytes"
                )
    ...
    def _parse_csv(self, file_path):
        ...
        try:
            ...
            if file_size > max_size:
                self.logger.warning(
                    f"CSV文件过大，跳过完整解析 {file_path}: {file_size} bytes"
                )
    ...
    def _parse_excel(self, file_path):
        ...
        try:
            ...
            if file_size > max_size:
                self.logger.warning(
                    f"Excel文件过大，跳过完整解析 {file_path}: {file_size} bytes"
                )
                        ...
                        try:
                            ...
                            if used_range.Value:
                                ...
                                for row in used_range.Value:
                                    ...
                                    if (
                                        len(text)
                                        > self.MAX_OUTPUT_SIZE_TEXT_MB * 1024 * 1024
                                    ):
                                        text += "\n... (内容已截断)"
                                        break
                        except Exception:
            ...
            if textract:
                try:
                    ...
                    self.logger.warning(
                        f"无法使用textract解析Excel文件 {file_path}: {str(te)}"
                    )
            # 如果没有textract或解析失败，返回错误信息
    ...
    def _extract_docx_metadata(self, file_path):
        ...
        try:
            ...
            if core_props.created:
                metadata["Word-创建时间"] = core_props.created.strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            if core_props.modified:
                metadata["Word-修改时间"] = core_props.modified.strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
