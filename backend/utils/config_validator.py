...
class ValidationResult:
    ...
    def add_error(self, message: str, section: Optional[str] = None,
                  key: Optional[str] = None, suggestion: Optional[str] = None,
    ...
    def add_warning(self, message: str, section: Optional[str] = None,
                    key: Optional[str] = None, suggestion: Optional[str] = None,
                    code: Optional[str] = None):
        """添加警告"""
        issue = ValidationIssue(
    ...
    def add_info(self, message: str, section: Optional[str] = None,
                 key: Optional[str] = None, suggestion: Optional[str] = None,
                 code: Optional[str] = None):
        """添加信息（向后兼容）"""
        issue = ValidationIssue(
...
class ConfigValidator:
    ...
    def _validate_ai_model_config(self, result: ValidationResult) -> None:
        ...
        if mode == 'api':
            ...
            api_key = api_config.get('api_key', '') if isinstance(api_config, dict) else ''
            if not api_key:
                result.add_warning(
            ...
            api_url = api_config.get('api_url', '') if isinstance(api_config, dict) else ''
            if not api_url:
                result.add_error(
...
def validate_config_on_startup(config_loader) -> bool:
    ...
    if result.has_errors():
        raise ConfigValidationError(
            "配置验证失败，请修复上述错误后重试",
            validation_errors=[
                {"code": i.code, "message": i.message, "section": i.section, "key": i.key}
